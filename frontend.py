...
def search_command():
    list1.delete(0,END)
    for row in backend.search(title_text.get(), artiste_text.get(), year_text.get(), genre_text.get()):
        list1.insert(END, row)

def add_command():
    backend.insert(title_text.get(), artiste_text.get(), year_text.get(), genre_text.get())
    list1.delete(0,END)
    list1.insert(END,(title_text.get(), artiste_text.get(), year_text.get(), genre_text.get()))

def delete_command():
    backend.delete(selected_tuple[0])

def update_command():
    backend.update(selected_tuple[0], title_text.get(), artiste_text.get(), year_text.get(), genre_text.get())
...
e2=Entry(window, textvariable= artiste_text)
